...
void ft_chk_mv2(t_vrb *vr, enum Ops *l_op)
{
	...
	{
		//заполним структуру
		ft_fl_str_vr(vr->st1, vr->st2);
		while (vrr.tmp)
		{
			ft_def_mv(vr->st1, vr->st2);
			vrr.tmp = vrr.tmp->next;
		}
		//ccl_def(st1, st2);
		//крутим стек 1 и стек 2, параллельно двигаем и закидываем элемент в стек 1
		while (vrr.c_op)
		{
			...
			if (vrr.dp1 && vrr.dp2 && (vrr.op2 - vrr.op1) == 1 \
				&& vrr.dp1-- && vrr.dp2-- && vrr.c_op--)
				ft_pushswap4_3(vr, (vrr.op2 + 1), l_op);
			else if (vrr.dp1 && vrr.dp1-- && vrr.c_op--)
				ft_pushswap4_3(vr, vrr.op1, l_op);
			else if (vrr.dp2 && vrr.dp2-- && vrr.c_op--)
				ft_pushswap4_3(vr, vrr.op2, l_op);
			else if (!vrr.dp1 && !vrr.dp2 && vrr.c_op--)
				ft_pushswap4_3(vr, PA, l_op);
			//wr_prl_st(*st1, *st2);
			//write(1, "\n", 1);
		}
		//ccl_mv(st1, st2, l_op);
